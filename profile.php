...
class profile
{
    ...
    public function format_phone($style = "default")
    {
        switch ($style) {
            case "plain":
    ...
    public function get_block_titles($zone)
    {
        global $con;
    ...
    public function print_blocks($zone)
    {
        $text = "";
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            if ($row['display_title']) {
                $text .= "<h2 id='".$row['machine_title']."' class='section-title'>".$row['title']."</h2>";
            }
    ...
    public function get_number_of_opinions()
    {
    ...
    public function print_opinions()
    {
        $text = "";
        ...
        if ($result) {
            while ($opinion = mysqli_fetch_array($result)) {
                ...
                $text .= "<div itemscope itemtype='http://schema.org/Review'>";
                $text .= '<span itemprop="itemreviewed" itemscope itemtype="http://schema.org/Person" style="display: none;"><span itemprop="name">'. $this->get_name() .'</span></span>';
                $lang = "";
                if ($opinion['lang'] != "") {
                ...
                if ($opinion['reviewer_url']) {
                    $reviewer = '<a href="'. $opinion['reviewer_url'] .'" itemprop="name">'. $opinion['reviewer'] .'</a>';
                } else {
                    $reviewer = '<span itemprop="name">' . $opinion['reviewer'] . '</span>';
                }
                if ($opinion['source_url']) {
                    $date = "<a href='" . $opinion['source_url'] . "'><time itemprop='dtreviewed' datetime='". $opinion['date'] ."'>". date("j/n/Y", $date) ."</time></a>";
                } else {
                    $date = date("j/n/Y", $date);
                }
                if ($opinion['reviewer_title']) {
                    $title = ", <em>" . $opinion['reviewer_title'] . "</em>";
                }
                $text .= '<p class="autor"><span itemprop="author" itemscope itemtype="http://schema.org/Person">' . $reviewer .'</span>'. $title . ' — ' . $date;
                if ($opinion['rating'] > 0) {
                    $text .= ' <span itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating">(<span itemprop="ratingValue">'. $opinion['rating'] .'</span>/<span itemprop="bestRating">10</span>)</span>';
                }
                $text .= "</div>";
                $text .= "</article>";
            }
        ...
        if ($result) {
            $opinions_numbers = mysqli_fetch_array($result);
            if ($opinions_numbers['rating'] > 0) {
                $ratings = "<span itemprop='rating' itemscope itemtype='http://data-vocabulary.org/Rating'>(<span itemprop='average'>". round(100*$opinions_numbers['rating'])/100 ."</span>/<span itemprop='best'>10</span>)</span>";
            }
        }
        if ($opinions_numbers['count'] > 5) {
            $text .= "<p><a id='show-more-opinions' title='Mostra més opinions' href='javascript:void(0)'>Mostra més opinions</a><span class='separator'> | </span><span itemprop='votes count'>". $opinions_numbers['count'] ."</span> opinions " . $ratings . "</p>";
        }

        return $text;
    }

    public function print_experience($volunteer = -1, $not_related = -1)
    {
        $text = "";
    ...
    public function print_work_professional_experience()
    {
        return $this->print_experience(0, 0);
    }

    public function print_work_not_professional_experience()
    {
        return $this->print_experience(0, 1);
    }

    public function print_work_experience()
    {
        return $this->print_experience(0);
    }

    public function print_volunteer_professional_experience()
    {
        return $this->print_experience(1, 0);
    }

    public function print_volunteer_not_professional_experience()
    {
        return $this->print_experience(1, 1);
    }

    public function print_volunteer_experience()
    {
        return $this->print_experience(1);
    }

    public function print_education()
    {
        $text = "";
    ...
    public function print_languages()
    {
        $text = "";
