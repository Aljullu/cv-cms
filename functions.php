...
function createProfile($uri = "")
{
    ...
    while ($row = mysqli_fetch_array($result)) {
        if ($my_profile->name != "") {
            die(__("Duplicated entry in database."));
        }
...
function format_dates($start, $finish)
{
    if ($start != "0000-00-00" && $start != "3000-01-01") {
        ...
        $start_formated = "<span class='dtstart'>Present</span>";
    }
    if ($finish != "0000-00-00" && $finish != "3000-01-01") {
        ...
        $finish_formated = "<span class='dtend'>" . __("Present") . "</span>";
    }
